...
# NOTE: This must be named the same as the Word2Vec model class in the model
#       repository.
class MalamudWord2Vec(Word2Vec):
    ...
    def get_n_closest(self,
        ...
        Raises:
            KeyError if the word is not in the model's vocabulary.
        """
        return [self.wv.most_similar(word, topn=n) for word in words]
    ...
    def complete_analogy(self, a: str, b: str, c: str, n: int) -> List[Tuple[str, float]]:
        ...
        Raises:
            KeyError if the word is not in the model's vocabulary.
        """
        return self.wv.most_similar(positive=[a, c], negative=[b], topn=n)
...
if __name__ == "__main__":
    m = MalamudWord2Vec.load("./trained_models/test/test.model")
